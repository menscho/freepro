...
pub fn main(init: std.process.Init) !void {
    ...
    var pool = engine.freeproxy.Pool.init(a, init.io);
    defer pool.deinit();
    ...
    while (ports.next()) |port| try pool.entries.append(a, .{ .host = try a.dupe(u8, "127.0.0.1"), .port = try std.fmt.parseInt(u16, port, 10) });
    ...
    var proxy = engine.proxy.Proxy.init(a, &config, .{ .io = init.io, .free_proxies = &pool });
    try proxy.start();
