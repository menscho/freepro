...
test "release versions exclude current older malformed and prereleases" {
    try std.testing.expect(newer("v0.1.6"));
    try std.testing.expect(newer("v1.0.0"));
    for ([_][]const u8{ "v0.1.5", "v0.1.4", "v0.1.3", "v0.1.0", "v0.0.9", "nope", "v0.2.0-beta.1" }) |v| try std.testing.expect(!newer(v));
}
...
test "release selection requires exact repository assets and checksum manifest" {
    ...
    const name = try std.fmt.allocPrint(a, "freepro-v0.1.6-{s}", .{platform()});
    const url = try std.fmt.allocPrint(a, "{s}v0.1.6/{s}", .{ repository, name });
    const valid = try std.json.Stringify.valueAlloc(a, .{ .tag_name = "v0.1.6", .assets = .{ .{ .name = name, .browser_download_url = url }, .{ .name = "SHA256SUMS", .browser_download_url = repository ++ "v0.1.6/SHA256SUMS" } } }, .{});
    try std.testing.expect((try selectRelease(a, valid)) != null);
    const missing = try std.json.Stringify.valueAlloc(a, .{ .tag_name = "v0.1.6", .assets = .{.{ .name = name, .browser_download_url = url }} }, .{});
    try std.testing.expect((try selectRelease(a, missing)) == null);
    const foreign = try std.json.Stringify.valueAlloc(a, .{ .tag_name = "v0.1.6", .assets = .{ .{ .name = name, .browser_download_url = "https://example.com/foreign" }, .{ .name = "SHA256SUMS", .browser_download_url = repository ++ "v0.1.6/SHA256SUMS" } } }, .{});
