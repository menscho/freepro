// Vendored from Zig 0.16.0 std/http/Client.zig (MIT; see zig-LICENSE).
// Local fix: HTTPS over an HTTP CONNECT tunnel must start origin TLS.
...
pub fn connect(
    ...
    // A failed CONNECT must never fall back to sending HTTPS plaintext.
    const tunnel = connectProxied(client, proxy, host, port) catch
        return error.ConnectionRefused;
    if (protocol == .plain) return tunnel;
    if (proxy.protocol != .plain) {
        tunnel.closing = true;
        client.connection_pool.release(tunnel, client.io);
        return error.TlsInitializationFailed;
    }
    const stream = tunnel.stream_reader.stream;
    client.connection_pool.mutex.lockUncancelable(client.io);
    client.connection_pool.used.remove(&tunnel.pool_node);
    client.connection_pool.mutex.unlock(client.io);
    const plain: *Connection.Plain = @alignCast(@fieldParentPtr("connection", tunnel));
    Connection.Plain.destroy(plain); // stream ownership transfers to TLS below
    errdefer stream.close(client.io);
    const tls = Connection.Tls.create(client, host, port, stream) catch |err| switch (err) {
        error.OutOfMemory => return error.OutOfMemory,
        error.Canceled => return error.Canceled,
        else => return error.TlsInitializationFailed,
    };
    client.connection_pool.addUsed(client.io, &tls.connection);
